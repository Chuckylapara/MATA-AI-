"""Text-to-video service (async jobs). Submit -> poll."""
...
from mata.common.schemas import JobOut, VideoRequest

app = create_app("Video")


@app.post("/jobs", response_model=JobOut, status_code=202)
async def submit(body: VideoRequest, identity: Identity = Depends(get_identity), db: AsyncSession = Depends(get_db)) -> JobOut:
    gen = await create_job(db, identity.user_id, "video", body.prompt, body.model_dump())
